...
type UserHdl interface {
	GetGorm(ctx *gin.Context)
	CreateGorm(ctx *gin.Context)
	Login(ctx *gin.Context)
}

func NewUserHdl(repo repository.UserRepo) UserHdl {
	return &userHdlImpl{Repository: repo}
}
...
func (u *userHdlImpl) GetGorm(ctx *gin.Context) {
...
// register user
func (u *userHdlImpl) CreateGorm(ctx *gin.Context) {
	// bind body
	user := &model.User{}
...
func (u *userHdlImpl) Login(ctx *gin.Context) {
	// bind body
	user := &model.User{}
	if err := ctx.Bind(user); err != nil {
		ctx.AbortWithStatusJSON(http.StatusBadRequest, map[string]any{
...
func (u *userHdlImpl) Get(ctx *gin.Context) {
	ctx.JSON(http.StatusOK, model.Users)
}

func (u *userHdlImpl) Create(ctx *gin.Context) {
	// bind body
	user := &model.User{}
	if err := ctx.Bind(user); err != nil {
		ctx.AbortWithStatusJSON(http.StatusBadRequest, map[string]any{
...
func (u *userHdlImpl) Update(ctx *gin.Context) {
	// getid from param
	userID := ctx.Param("id")
	if userID == "" {
		ctx.AbortWithStatusJSON(http.StatusBadRequest, map[string]any{
